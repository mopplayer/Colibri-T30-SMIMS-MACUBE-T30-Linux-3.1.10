...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	//=====================================
	// 2013.12.12 added by SMIMS ikki
	// we put a compiler control here
#ifndef CONFIG_SMIMS_FPGA_Platform
	tegra_host->card_present = (gpio_get_value(plat->cd_gpio) == 0); //original
#else
	tegra_host->card_present = (gpio_get_value(plat->cd_gpio) == 1); // ikki modify
#endif
	//pr_info("[smimsfpga] sdhci-tagra.c carddetect_irq() gpio_get_value(plat->cd_gpio) = %d\n", gpio_get_value(plat->cd_gpio));
	//pr_info("[smimsfpga] sdhci-tagra.c carddetect_irq() tegra_host->card_present = %d\n", tegra_host->card_present);
	//=====================================
