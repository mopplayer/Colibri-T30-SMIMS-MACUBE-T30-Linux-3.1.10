...
//===========================================================
// 2013.12.10 added by SMIMS ikki
static struct resource smimsfpga_resources[] = {
		[0] = {
				.start  = 0x48000000,
				.end    = 0x48000000 + 0x10000 - 1,
				.flags  = IORESOURCE_MEM,				
		},
		[1] = {
				.start  = TEGRA_GPIO_TO_IRQ(TEGRA_GPIO_PJ0),
				.end    = TEGRA_GPIO_TO_IRQ(TEGRA_GPIO_PJ0),
				.flags  = IORESOURCE_IRQ,
				.name	= "smimsfpga_irq1",
		}, 
		[2] = {
				.start  = TEGRA_GPIO_TO_IRQ(TEGRA_GPIO_PJ2),
				.end    = TEGRA_GPIO_TO_IRQ(TEGRA_GPIO_PJ2),
				.flags  = IORESOURCE_IRQ,
				.name	= "smimsfpga_irq2",
		} 
};

struct platform_device smimsfpga_device = {
		.name           = "smimsfpga",
		.id             = -1,
		.num_resources  = ARRAY_SIZE(smimsfpga_resources),
		.resource       = smimsfpga_resources,
};

static void __init colibri_t30_smimsfpga_irq_init(void)
{
	if (gpio_request(TEGRA_GPIO_PJ0, "smimsfpga_irq1") == 0)
	{
		if (gpio_direction_input(TEGRA_GPIO_PJ0) == 0)
		{
			printk("[smimsfpga] gpio_direction_input for TEGRA_GPIO_PJ0 ok\n");
		}
		else
		{
			printk(KERN_ERR "[smimsfpga] gpio_direction_input for TEGRA_GPIO_PJ0\n");
		}
	} 
	else 
	{
		printk(KERN_ERR "[smimsfpga] Could not obtain TEGRA_GPIO_PJ0 GPIO for smimsfpga_irq1\n");
		return;
	}

	if (gpio_request(TEGRA_GPIO_PJ2, "smimsfpga_irq2") == 0)
	{
		if (gpio_direction_input(TEGRA_GPIO_PJ2) == 0)
		{
			printk("[smimsfpga] gpio_direction_input for TEGRA_GPIO_PJ2 ok\n");
		}
		else
		{
			printk(KERN_ERR "[smimsfpga] gpio_direction_input for TEGRA_GPIO_PJ2\n");
		}
	} 
	else 
	{
		printk(KERN_ERR "[smimsfpga] Could not obtain TEGRA_GPIO_PJ2 GPIO for smimsfpga_irq2\n");
		return;
	}
...
static struct platform_device *colibri_t30_devices[] __initdata = {
	...
	//==================================
	// 2013.12.10 added by SMIMS ikki
	&smimsfpga_device, //SMIMS FPGA IF
	//==================================
};
...
//	colibri_t30_sensors_init();
	...
	//=====================================
	// 2013.12.10 added by SMIMS ikki
	colibri_t30_smimsfpga_irq_init();
//#ifdef COLIBRI_T30_V10
//	pr_info("colibri_t30_init() defined COLIBRI_T30_V10\n");
//#else
//	// not define
//	pr_info("colibri_t30_init() not define COLIBRI_T30_V10\n");
//#endif
//#ifdef COLIBRI_T30_SDMMC4B
//	pr_info("colibri_t30_init() defined COLIBRI_T30_SDMMC4B\n");
//#else
//	// not define
//	pr_info("colibri_t30_init() not define COLIBRI_T30_SDMMC4B\n");
//#endif
	//=====================================
